...
app.post('/login/', async (request, response) => {
  ...
  if (userDetails !== undefined) {
    ...
    if (isPasswordValid) {
      ...
      const jwtToken = jwt.sign(payload, 'kumar_secret_token')
...
function authenticationToken(request, response, next) {
  ...
  if (jwtToken !== undefined) {
    jwt.verify(jwtToken, 'kumar_secret_token', async (error, payload) => {
      if (error) {
        response.status(401)
...
app.get(
  ...
  async (request, response) => {
    ...
    const getDistrictByIdQueryResponse =
      await database.get(getDistrictByIdQuery)
